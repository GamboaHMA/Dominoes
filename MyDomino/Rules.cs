...
namespace MyDomino
{
    public class Rules: IRules<TokenDom, (int, int)>
    {
        ...
        public void DistributeTokens(List<TokenDom> tokens, IPlayer<TokenDom, (int, int)>[] players, int cant)
        {
            if (cant * players.Length > tokens.Count) throw new Exception("The tokens are not enough");
        ...
        public bool GameOver(IBoard<TokenDom, (int, int)> board, IPlayer<TokenDom, (int, int)>[] players)
        {
            int count = 0;
            foreach (var player in players)
            {
        ...
        public IPlayer<TokenDom, (int, int)>[] Winners(IBoard<TokenDom, (int, int)> board, IPlayer<TokenDom, (int, int)>[] players)
        {
            int value = 0;
            ...
            {
                if (players[i].GetHand().Count == 0) { winners = new Player[1]; winners[0] = players[i]; return winners; }
            ...
            winners = new Player[1]; winners[0] = players[index];
            return winners;
        }
    }
}
